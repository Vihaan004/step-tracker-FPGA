...
module top(
    ...
    bin2bcd_fsm step_count_converter(.clk100Mhz(clk100Mhz), .rst(rst), .start(pulse), .bin(step_count), .bcd(step_count_bcd));
    bin2bcd_fsm distance_converter(.clk100Mhz(clk100Mhz), .rst(rst), .start(pulse), .bin(mile), .bcd(distance_bcd));
